...
const MAP_MARKS = "⁂⁃⁄⁅⁆⁇⁈⁉⁊⁋";   // tracks, hive, web, nest, then the six town objects
...
// ---------- WHAT THE ROAD LEAVES IN THE TOWN ----------
// Ayr: "the towns are a little empty now that the buildings aren't in groups of
// 4. please add stuff related to each route to them."
//
// She is right about the cause. Unstacking the buildings took 164 duplicate
// tiles out of nine towns, which was the correct fix - four arena tiles was one
// arena drawn four times - but it left the floor they stood on bare.
//
// What goes back is deliberately ordinary: crates, a basket, a barrel, a pail,
// a bench, a toolbox. A crate is a crate in every town on earth, and if each
// town got its own exotic furniture the whole set would read as decoration.
// What makes each one belong to its road is the line you get for walking into
// it. Every town is the far end of exactly one road, every road already has a
// subject, and the town is where you see what that road has done to the people
// living at the end of it. Marula Town has hive frames nobody will put back
// out. Delta Town hands out bells to wire onto pail handles. Gloam Town has a
// bench under the one lamp that still does not work.
//
// They are floor, not obstacles. Marks have been walkable since the "I'm stuck"
// fix, and that is the whole reason nothing added here can close a town off.
const TOWN_SPOTS = {
  town1: [   // Baobab Base - where the bee road starts
    ["⁆", "📦 Crates by the gate, addressed north. Empty hive frames going up to the apiaries, and a note on the top one: NO SMOKE UNTIL I GET THERE."],
    ["⁇", "🧺 A basket of marulas gone soft, smelling like a brewery. Somebody's payment or somebody's dinner, depending how the week went."],
    ["⁊", "🪑 A bench worn to a dip in the middle. From here you can see the whole road out, which is presumably why it faces that way."],
    ["⁉", "🪣 A pail of water left out for whatever comes through. The rim is scratched where something stood on it to drink."],
  ],
  town2: [   // Marula Town - Thabo's hives, and the badger feeding two cubs
    ["⁆", "📦 Hive frames stacked six high against the wall, every one of them empty. Nobody is putting boxes back on the outer rows until somebody works out what keeps opening them."],
    ["⁈", "🛢️ A honey drum, tapped and half full. The label gives a weight for the season, and the weight has been crossed out twice, smaller each time."],
    ["⁋", "🧰 A toolbox open on the ground - tin snips, wire, a roll of smooth sheet steel. Somebody has been reading about what a badger cannot climb."],
    ["⁊", "🪑 A bench outside the clinic, the paint worn off one armrest and not the other. People wait here a long time, and always sit the same way round."],
  ],
  town3: [   // Delta Town - the fen cats and the ground-nesters
    ["⁈", "🛢️ A rain barrel with its lid wired shut. Out here anything left open is a nest by morning and a meal by the morning after."],
    ["⁇", "🧺 A basket of reed cuttings, still green. Somebody is rethatching now, before the birds are down, rather than in spring when they are."],
    ["⁉", "🪣 A pail with a bell wired to the handle. The warden hands them out - the bell goes on the cat, and the birds get a second of warning."],
    ["⁊", "🪑 A bench facing the water. Three names are scratched into the back and one of them has been scratched out again."],
  ],
  town4: [   // Canopy Town - the plantation cleared its snakes, and the rats came
    ["⁆", "📦 Grain crates, and every one of them stands on a metal cone the width of a dinner plate. The cones are new. So are the rats."],
    ["⁋", "🧰 A toolbox of wire and mesh and nothing else. Nobody here is building anything at the moment; they are all sealing things shut."],
    ["⁇", "🧺 A basket of chewed sacking, kept rather than thrown out. It gets passed round at meetings, and it has never once needed explaining."],
    ["⁊", "🪑 A bench directly under where the walkway ropes cross. Sitting here means accepting that things will go over your head all afternoon."],
  ],
  town5: [   // Dune Town - turtles drowning in the prawn nets
    ["⁈", "🛢️ A water barrel, chained to the post. The chain is not really about the barrel."],
    ["⁆", "📦 Crates of net floats, and beside them something still banded up - a hoop with a grid across it. Whoever ordered it has not opened it yet."],
    ["⁉", "🪣 A pail of sand with a thermometer standing upright in it. Somebody is finding out how hot the nests are running, one bucketful at a time."],
    ["⁊", "🪑 A bench in the only shade this side of the square. You can see the four grooves where it has been dragged round to follow it."],
  ],
  town6: [   // Crag Town - pangolins and the bottom wire
    ["⁋", "🧰 A toolbox of fencing tools. The wire strainer on top is polished smooth at the handle - twenty kilometres of fence is a job that never actually finishes."],
    ["⁆", "📦 Crates of insulators, still banded. Somebody has ordered enough to lift the bottom strand along the whole southern line."],
    ["⁇", "🧺 A basket with a jar inside it, and in the jar a handful of scales the size of thumbnails. Nobody in town will say who brought it in."],
    ["⁊", "🪑 A bench at the lookout, with the best view in the province. It is always empty by ten, because by ten there is no shade on it."],
  ],
  town7: [   // Frost Town - the wolves are back on the high pasture
    ["⁆", "📦 Crates of fladry - orange flags strung along a long line. It works for about a season, which is longer than nothing works for."],
    ["⁇", "🧺 A basket of wool, unspun and still smelling of the animal. Two fleeces are stained dark down one side and have been set apart from the rest."],
    ["⁋", "🧰 A toolbox with a radio collar lying in the lid, dead and dented. It is kept where everyone can see it, and it means different things to different people here."],
    ["⁊", "🪑 A bench against the south wall, where the sun lands first. It gets cleared of snow before anything else in the square does."],
  ],
  town8: [   // Cinder Town - the burning, and the rabbits that live in the bunchgrass
    ["⁈", "🛢️ A drum of water on a sled, standing ready by the door. In this town the fire is not an emergency, it is a date in the calendar."],
    ["⁆", "📦 Crates of seedlings - bunchgrass plugs by the hundred. Somebody is putting back by hand, one plug at a time, what goes off the hill every season."],
    ["⁊", "🪑 A bench cut from a slab of black glass out of the field. Cold to sit on, and still sharp along one edge where nobody has got round to it."],
    ["⁉", "🪣 A pail of ash part way through being sieved. Whatever they are looking for in it, they have not found it yet."],
  ],
  town9: [   // Gloam Town - horseshoe bats, and the new lighting on the grove road
    ["⁊", "🪑 A bench under the old lamp, which is out. The new ones are further along, and nobody sits under those."],
    ["⁋", "🧰 A toolbox of lamp parts - shades, hoods, one amber sleeve still in its wrapper. Somebody is trying to put the dark back a bit at a time."],
    ["⁆", "📦 Crates of the new fittings, half of them still unopened. The order went in before anybody had counted what was flying over that road."],
    ["⁇", "🧺 A basket of moths, dead, sorted onto card by size. It is a tally rather than a collection, and it is getting shorter."],
  ],
};

// What each placed object says. Keyed by tile rather than by character, because
// the same crate stands in six towns and says something different in each.
const SPOT_LINE = {};

const placeTownSpots = () => {
  if (typeof MAPS === "undefined") return 0;
  let placed = 0;
  Object.keys(TOWN_SPOTS).forEach((mk) => {
    const m = MAPS[mk];
    if (!m || !m.rows) return;
    const H = m.rows.length, W = m.rows[0].length;

    // Nowhere a traveller gets put down. A prop on a landing tile would fire
    // its line every single time the player walked into town, which turns a
    // piece of scenery into a doorbell.
    const keepClear = [];
    if (typeof LAND !== "undefined") {
      Object.keys(LAND).forEach((k) => keepClear.push(LAND[k].join(",")));
    }
    (m.exits ? Object.keys(m.exits) : []).forEach((e) => keepClear.push(e));

    // Hash the whole key. "town1" through "town9" are all five characters
    // long, so seeding off mk.length gave every town in the game the same four
    // start positions and therefore very nearly the same four spots - which is
    // the repeated-icon look that unstacking the buildings was meant to end.
    let mh = 0;
    for (let i = 0; i < mk.length; i++) mh = (Math.imul(mh, 31) + mk.charCodeAt(i)) | 0;

    const taken = [];
    TOWN_SPOTS[mk].forEach(([ch, line], i) => {
      // Deterministic, and each object starts its search from a different part
      // of its own town, so the four do not end up in a row along the top the
      // way the first draft of the fruit did.
      const y0 = 1 + Math.floor(ambSeed(i * 3 + 1, mh ^ 3307) * (H - 2));
      const x0 = 1 + Math.floor(ambSeed(i * 5 + 2, mh ^ 7717) * (W - 2));
      let spot = null;
      for (let dy = 0; dy < H - 2 && !spot; dy++) {
        const y = 1 + ((y0 - 1 + dy) % (H - 2));
        for (let dx = 0; dx < W - 2 && !spot; dx++) {
          const x = 1 + ((x0 - 1 + dx) % (W - 2));
          if (m.rows[y][x] !== ".") continue;
          // Clear of the landings and their elbow room, and well apart from the
          // other three, so a town gets furniture rather than a pile of it.
          if (keepClear.some((c) => {
            const p = c.split(",");
            return Math.abs(Number(p[0]) - x) + Math.abs(Number(p[1]) - y) <= 1;
          })) continue;
          if (taken.some((t) => Math.abs(t[0] - x) + Math.abs(t[1] - y) < 4)) continue;
          spot = [x, y];
        }
      }
      if (!spot) return;
...
console.log("[part67] town objects placed:", placeTownSpots());
