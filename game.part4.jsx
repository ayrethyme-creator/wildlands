...
function Wildlands() {
  ...
  const move = (dx, dy) => {
    ...
    if (walk) {
      ...
      // ONLY THE TRACKS SPEAK. Ayr, over 2026-09-04: "the bird nests and spider
      // web should not stop you. the animal tracks makes sense", then "please
      // just get rid of that function", then "the hive should also not do that."
      //
      // The hive, the web and the nest are set dressing and say nothing at all
      // now - part67 no longer gives them a line, and DRESSING_LINE is gone with
      // it. Tracks stay because readTracks names who is living on THIS map right
      // now: it answers a question rather than reciting a fixed paragraph, which
      // is the distinction Ayr drew every time.
      //
      // Two intermediate versions of this are worth not repeating. Showing a
      // fixed line ONCE per tile and remembering it in the save looked like a
      // compromise and was not one: there is one of each per map, so every new
      // map was a first crossing and it fired about as often as before. And the
      // read-once bookkeeping that went with it is gone too - nothing needs
      // remembering when nothing is said.
