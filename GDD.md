...
| 7b | ~~**The Opah has no IUCN status**~~ — **RULED, Ayr 2026-09-02: Least Concern.** It was the only one of the forty-four whose status field was absent, and `game.part68.jsx` now carries it on both games | No |
| 7c | ~~**1000 descriptions**~~ — **DONE, 2026-09-02.** All 1000 Terrane species have a field-guide entry: taxon, diet, habitat, IUCN status and a written fact. The pipeline is empty — no `new>` line left in `PENDING_MOVES.txt`. Safari Saga holds 1183, and the thousand no longer applies to it | No |
